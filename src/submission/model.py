...
class DownProjectBlock(nn.Module):
    ...
    def forward(self, x_input):
        ...
        ### START CODE HERE 
        cross_attn_output = self.cross_attn(self.ln1(x_input), self.ln1(self.C))        
        cross_attn_output = self.ln2(cross_attn_output + x_input)
        return self.mlp(cross_attn_output)
        ### END CODE HERE
...
class UpProjectBlock(nn.Module):
    ...
    def forward(self, y, x_input):
        ...
        ### START CODE HERE 
        cross_attn_output = self.cross_attn(self.ln1(y), self.ln1(x_input))
        cross_attn_output = self.ln2(cross_attn_output + y)
        return self.mlp(cross_attn_output)
        ### END CODE HERE
