...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        masked_length = max(1, length // 4)  
        prefix_length = random.randint(1, length - masked_length)
        ...
        # Creating masked string format
        masked_string = f"{prefix}{self.MASK_CHAR}{suffix}{self.MASK_CHAR}{masked_content}{self.MASK_CHAR}"
        pads_length = self.block_size - len(masked_string)
        pads = self.PAD_CHAR * pads_length
        masked_string += pads

        # Encode input and output sequences
        input_encoded = [self.stoi[char] for char in masked_string[:-1]]
        output_encoded = [self.stoi[char] for char in masked_string[1:]]

        # Creating long tensors
        input_tensor = torch.tensor(input_encoded, dtype=torch.long)
        output_tensor = torch.tensor(output_encoded, dtype=torch.long)
